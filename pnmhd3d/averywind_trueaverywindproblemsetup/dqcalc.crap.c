...
void dqx_calc(FTYPE (*var)[N2M][N1M],FTYPE (*dq)[N2M][N1M])
{
  ...
  if(advint==2){
    LOOPHC{
      ...
      Dqp = (var[k][j][ip1] - var[k][j][i])*OARC11(k,j,ip1mac(i));
      WOODDQ(dq)
    }// end looph
...
void dqvx_calc(int wcom, FTYPE (*var)[N3M][N2M][N1M],FTYPE (*dqv)[N3M][N2M][N1M])
{
  ...
  if(advint==2){
    if(wcom==1){
      LOOPHC{
	Dqm = (var[wcom][k][j][i] - var[wcom][k][j][im1])*OARC21(k,j,im1mac(i));
	...
	Dqp = (var[wcom][k][j][ip1] - var[wcom][k][j][i])*OARC31(k,j,ip1mac(i));
	WOODDQ(dqv[wcom])
      }// end looph
    } // end if wcom==2
    else if(wcom==3){
	...
	Dqc = (var[wcom][k][j][ip1] - var[wcom][k][j][im1])/(dx[2][1][ip1]+dx[2][1][i]);
	Dqp = (var[wcom][k][j][ip1] - var[wcom][k][j][i])*OARC11(k,j,ip1mac(i));
	WOODDQ(dqv[wcom])
      }// end looph
    } // end if wcom==3
  } // end if woodward slope
...
void dqy_calc(FTYPE (*var)[N2M][N1M],FTYPE (*dq)[N2M][N1M])
{
  ...
  if(advint==2){
    LOOPHC{   // special for periodicx2special in sweepy(vy) (hence bound)
      ...
      Dqp = (var[k][jp1][i] - var[k][j][i])*OARC12(k,jp1mac(j),i);
      WOODDQ(dq)
    }
  }
...
void dqvy_calc(int wcom,FTYPE (*var)[N3M][N2M][N1M],FTYPE (*dqv)[N3M][N2M][N1M])
{
  ...
  if(advint==2){
	...
	Dqp = (var[wcom][k][jp1][i] - var[wcom][k][j][i])*OARC22(k,jp1mac(j),i);
	...
	Dqm = (var[wcom][k][j][i] - var[wcom][k][jm1][i])*OARC32(k,jm1mac(j),i);
	...
	Dqp = (var[wcom][k][jp1][i] - var[wcom][k][j][i])*OARC12(k,jp1mac(j),i);
	WOODDQ(dqv[wcom])
      }
    }
  }// end if woodward
...
void dqz_calc(FTYPE (*var)[N2M][N1M],FTYPE (*dq)[N2M][N1M])
{
  ...
  if(advint==2){
    LOOPHC{
      ...
      Dqc = (var[kp1][j][i]-var[km1][j][i])*OARC13(k,j,i)*(ODX(2,3,kp1mac(k))+ODX(2,3,k));
      Dqp = (var[kp1][j][i] - var[k][j][i])*OARC13(kp1mac(k),j,i)*ODX(2,3,kp1mac(k));
      WOODDQ(dq)
    }
  }
  else if(advint==1){
...
void dqvz_calc(int wcom,FTYPE (*var)[N3M][N2M][N1M],FTYPE (*dqv)[N3M][N2M][N1M])
{
  ...
  if(advint==2){
	...
	Dqc = (var[wcom][kp1][j][i] - var[wcom][km1][j][i])*OARC23(k,j,i)*(ODX(2,3,kp1mac(k))+ODX(2,3,k));
	Dqp = (var[wcom][kp1][j][i] - var[wcom][k][j][i])*OARC23(kp1mac(k),j,i)*ODX(2,3,kp1mac(k));
	...
	Dqc = (var[wcom][kp1][j][i] - var[wcom][km1][j][i])*OARC33(k,j,i)*(ODX(2,3,kp1mac(k))+ODX(2,3,k));
	Dqp = (var[wcom][kp1][j][i] - var[wcom][k][j][i])*OARC33(kp1mac(k),j,i)*ODX(2,3,kp1mac(k));
	...
	WOODDQ(dqv[wcom])
      ...
      LOOPHC{
	Dqm = (var[wcom][k][j][i] - var[wcom][km1][j][i])*OARC13(km1mac(k),j,i)*ODX(1,3,km1mac(k));
	Dqc = (var[wcom][kp1][j][i] - var[wcom][km1][j][i])*OARC13(k,j,i)*(ODX(1,3,k)+ODX(1,3,km1mac(k)));
	...
	Dqm = (var[wcom][k][j][i] - var[wcom][km1][j][i])*OARC33(km1mac(k),j,i)*ODX(1,3,km1mac(k));
	VLDQ(dqv[wcom])
#if(DQCURVATURE&&(COORD>1))
