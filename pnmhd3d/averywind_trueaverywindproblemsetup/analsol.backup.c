...
FILE * tori1sol(int calltype)
{
  ...
  MAGTORI=1; // 1: include magnetic equadensity rings 0: don't have any magnetic field
  ...
  if(calltype==100){
	...
	*/
    ...
    if(TORITYPE==0){
      // viscosity run
      //tvisc=2.0*M_PI*pow(R0,1.5)/alpha_real0; //viscous time scales
      //numvtime=2.0;
...
#define READINFILE 1
	...
	//	  sprintf(manuelfile3,"%s","velocities.in");
	//  fprintf(stdout,"%s",manuelfile3);
	//
        //if((manuel=fopen(manuelfile3,"r"))==NULL){
	// fprintf(fail_file,"Cannot open %s\n",manuelfile3);
	//  myexit(1);
	//
	//}
    ...
    //int dumi[1],dumi[2],dumi[3];
    //double dumf[1],dumf[2],dumf[3],dumf[4],dumf[5],dumf[6],dumf[7],dumf[8],dumf[9],dumf[10],dumf[11];
    //int ta,tb,tc;
    //double td,te,tf,tg,th,ti,tj,tk,tl,tm,tn;
    ...
    LOOP{
      ...
      //     fscanf(manuel,"%d %d %d %g %g %g %g %g %g %g %g %g %g %g\n",&dumi[1],&dumi[2],&dumi[3],&dumf[1],&dumf[2],&dumf[3],&dumf[4],&dumf[5],&dumf[6],&dumf[7],&dumf[8],&dumf[9],&dumf[10],&dumf[11]);  

      // fscanf(fp,"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",dumi[1],dumi[2],dumi[3],dumf[1],dumf[2],dumf[3],dumf[4],dumf[5],dumf[6],dumf[7],dumf[8],dumf[9],dumf[10],dumf[11]);

      //   fscanf(fp,"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",ta,tb,tc,td,te,tf,tg,th,ti,tj,tk,tl,tm,tn);

      // fprintf(stdout,"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",ta,tb,tc,td,te,tf,tg,th,ti,tj,tk,tl,tm,tn);

      //    fprintf(stdout,"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",&dumi[1],&dumi[2],&dumi[3],&dumf[1],&dumf[2],&dumf[3],&dumf[4],&dumf[5],&dumf[6],&dumf[7],&dumf[8],&dumf[9],&dumf[10],&dumf[11]);

      //      fprintf(stdout,"%d %d %d %g %g %g %g %g %g %g %g %g %g %g\n",i,j,k,0.0,0.0,0.0,vx[ijk],vy[ijk],vz[ijk],rho[ijk],2.5*\
	//	      press[ijk],-GCONV*mofr[ijk]*x/pow(radius,3.0),-GCONV*mofr[ijk]*y/pow(radius,3.0),-GCONV*mofr[ijk]*z/pow(radius,3.0));

			  // switch 'manuel' with 'velinput'
        ...
        fprintf(stdout,"%d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n",&dumi[1],&dumi[2],&dumi[3],&dumf[1],&dumf[2],\
		&dumf[3],&dumf[4],&dumf[5],&dumf[6],&dumf[7],&dumf[8],&dumf[9],&dumf[10],&dumf[11]);   
	...
	myexit(1);
      ...
      sanal[3][dumi[3]][dumi[2]][dumi[1]]=0.0; // grav. pot. (could try using GRAVACC)

      if(GRAVACC&&(!POSTPROC)){
	for(l=1;l<=3;l++){
	  gravacc[l][k][j][i]=dumf[8+l];
	}
      }
