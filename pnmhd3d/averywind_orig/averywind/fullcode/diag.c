...
#define WHICHDUM (dumlf)
					...
					if(analoutput!=13){
						if(!dumpsmdump){
							// rho
							writebuf[BUFFERMAP]=scain[1][k][j][i];
							// en
							writebuf[BUFFERMAP]=scain[2][k][j][i];
							// pot
							writebuf[BUFFERMAP]=scain[3][k][j][i];
							if(which==FLTYPE){
								writebuf[BUFFERMAP]=floorvar0[1][k][j][i];
							}
							// v1
							writebuf[BUFFERMAP]=vecin[1][1][k][j][i];
							// v2
							writebuf[BUFFERMAP]=vecin[1][2][k][j][i];
							// v3
							writebuf[BUFFERMAP]=vecin[1][3][k][j][i];
							if(which==FLTYPE){
								writebuf[BUFFERMAP]=floorvar0[2][k][j][i];
							}
							// B1
							writebuf[BUFFERMAP]=vecin[2][1][k][j][i];
							// B2
							writebuf[BUFFERMAP]=vecin[2][2][k][j][i];
							// B3
							writebuf[BUFFERMAP]=vecin[2][3][k][j][i];
						}
						else{
							// rho
							writebuf[BUFFERMAP]=scain[1][k][j][i];
							...
							writebuf[BUFFERMAP]=scain[3][k][j][i];
							// v1
							writebuf[BUFFERMAP]=e2z_1(vecin[1][1],k,j,i);
							// v2
							writebuf[BUFFERMAP]=e2z_2(vecin[1][2],k,j,i);
							// v3
							writebuf[BUFFERMAP]=e2z_3(vecin[1][3],k,j,i);
							// B1
							writebuf[BUFFERMAP]=e2z_1(vecin[2][1],k,j,i);
							// B2
							writebuf[BUFFERMAP]=e2z_2(vecin[2][2],k,j,i);
							// B3
							writebuf[BUFFERMAP]=e2z_3(vecin[2][3],k,j,i);
						}
					}
					else{
						if(!dumpsmdump){
				...
				if(VISCMEM){
					...
					if(mpicombine==0){
						for(l=1;l<=6;l++){ // number of components in sym tensor
							fprintf(fp,"%21.15g ",ftempT[l]);
						}
						fprintf(fp,"%21.15g ",nu_real[k][j][i]);	
					}
					else{
						for(l=1;l<=6;l++){ // number of components in sym tensor
							writebuf[BUFFERMAP]=ftempT[l];
						}
						writebuf[BUFFERMAP]=nu_real[k][j][i];
					}
				}// if sigma dump
				if(1){ //forces,fluxes, and energy
					if(mpicombine==0){
						for(l=1;l<=18;l++){// number of terms
							fprintf(fp,"%21.15g ",ffv_calcs(1,l,i,j,k));
						}
						for(l=1;l<=36;l++){// number of terms
							fprintf(fp,"%21.15g ",ffv_calcs(2,l,i,j,k));
						}
						for(l=1;l<=18;l++){// number of terms
							fprintf(fp,"%21.15g ",ffv_calcs(3,l,i,j,k));
						}
					}
					else{
						for(l=1;l<=18;l++){// number of terms
							writebuf[BUFFERMAP]=ffv_calcs(1,l,i,j,k);
						}
						for(l=1;l<=36;l++){// number of terms
							writebuf[BUFFERMAP]=ffv_calcs(2,l,i,j,k);
						}
						for(l=1;l<=18;l++){// number of terms
							writebuf[BUFFERMAP]=ffv_calcs(3,l,i,j,k);
						}
					}
				}
      }
				...
				if(mpicombine==0){
					fprintf(fp,"%21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g",avg2_3[1][j][i],avg2_3[2][j][i],avg2_3[3][j][i],avg2_3[4][j][i],avg2_3[5][j][i],avg2_3[6][j][i],avg2_3[7][j][i],avg2_3[8][j][i],avg2_3[9][j][i],avg2_3[10][j][i],avg2_3[11][j][i],avg2_3[12][j][i],avg2_3[13][j][i],avg2_3[14][j][i],avg2_3[15][j][i]);
				}
				else{
					for(l=1;l<=15;l++){
						writebuf[BUFFERMAP]=avg2_3[l][j][i]; // only applies in 2D
					}
				}
      }// if avg2d dump
				...
				if(mpicombine==0){
					fprintf(fp,"%21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g %21.15g",ppcalcs[1][k][j][i],ppcalcs[2][k][j][i],ppcalcs[3][k][j][i],ppcalcs[4][k][j][i],ppcalcs[5][k][j][i],ppcalcs[6][k][j][i],ppcalcs[7][k][j][i],ppcalcs[8][k][j][i],ppcalcs[9][k][j][i],ppcalcs[10][k][j][i],ppcalcs[11][k][j][i],ppcalcs[12][k][j][i]);
				}
				else{
					for(l=1;l<=12;l++){
						writebuf[BUFFERMAP]=ppcalcs[l][k][j][i];
					}
				}
				...
				if(mpicombine==0){
					...
					fprintf(fp,"%.6g",ax3[k][j][i]); // assume for now don't compute anything from it, just display, so don't need all precision
				}
				else{
					writebuf[BUFFERMAP]=ax3[k][j][i];
				}
  ...
  //
    ...
    LOOPD{
      ...
      if(SAMEGRIDD>0){
				...
				if( (which==DTYPE)||(which==PDTYPE)||(which==ADTYPE)||(which==FLTYPE) ){
					for(l=1;l<=NUMSCA;l++){
						fprintf(fp,"%21.15g ",iq[l][j][i]);
					}
					for(l=1;l<=REALNUMVEC;l++){
						if(which==FLTYPE){
							fprintf(fp,"%21.15g ",work0iq[l][j][i]);
						}
						if( (COORD==3)&&(outtype>0) ){
							// coords go to z vs x in 2D in mapping, y is flat coord
							fprintf(fp,"%21.15g ",ftemp[l][1]); // value interpolated and mapped, so need ftemp
							fprintf(fp,"%21.15g ",ftemp[l][3]); // value interpolated and mapped, so need ftemp
							fprintf(fp,"%21.15g ",ftemp[l][2]); // value interpolated and mapped, so need ftemp
						}
						else{
							for(m=1;m<=3;m++){
								fprintf(fp,"%21.15g ",ftemp[l][m]);
							}
						}	      
					}
				}
					...
					if( (COORD==3)&&(outtype>0) ){
						...
						fprintf(fp,"%21.15g ",ftempT[1]);
						fprintf(fp,"%21.15g ",ftempT[3]);
						fprintf(fp,"%21.15g ",ftempT[2]);
						fprintf(fp,"%21.15g ",ftempT[6]);
						fprintf(fp,"%21.15g ",ftempT[5]);
						fprintf(fp,"%21.15g ",ftempT[4]);
					}
					else{
						for(l=1;l<=6;l++){ // number of components in sym tensor
							fprintf(fp,"%21.15g ",ftempT[l]);
					...
					fprintf(fp,"%21.15g ",iq[1][j][i]);  // interp of nuvisc
				}// if sigma
				else if(which==AVG2DTYPE){
					for(l=1;l<=5;l++){
						fprintf(fp,"%21.15g ",workiqavg[l][j][i]);
					}
					if( (COORD==3)&&(outtype>0) ){
						// coords go to z vs x in 2D in mapping, y is flat coord
						fprintf(fp,"%21.15g ",ftemp[1][1]); // value interpolated and mapped, so need ftemp
						fprintf(fp,"%21.15g ",ftemp[1][3]); // value interpolated and mapped, so need ftemp
						fprintf(fp,"%21.15g ",ftemp[1][2]); // value interpolated and mapped, so need ftemp
					}
					else{
						for(m=1;m<=3;m++){
							fprintf(fp,"%21.15g ",ftemp[1][m]);
						}
					}
					if( (COORD==3)&&(outtype>0) ){
						// mapping of r,th,phi to x,z,y leads to this ordering change
					...
					fprintf(fp,"%21.15g ",workiqavg[15][j][i]);  // interp of nuvisc
					...
					for(m=1;m<=10;m+=3){
						if( (COORD==3)&&(outtype>0) ){
							// coords go to z vs x in 2D in mapping, y is flat coord
							fprintf(fp,"%21.15g ",ftempC[m]); // value interpolated and mapped, so need ftemp
							fprintf(fp,"%21.15g ",ftempC[m+2]); // value interpolated and mapped, so need ftemp
							fprintf(fp,"%21.15g ",ftempC[m+1]); // value interpolated and mapped, so need ftemp
						}
						else{
							fprintf(fp,"%21.15g ",ftempC[m]);
							fprintf(fp,"%21.15g ",ftempC[m+1]);
							fprintf(fp,"%21.15g ",ftempC[m+2]);
						}
					}
					...
					for(m=1;m<=3;m+=3){
						if( (COORD==3)&&(outtype>0) ){
							...
							// coords go to z vs x in 2D in mapping, y is flat coord
							//	      fprintf(fp,"%21.15g ",ftempC[m+1]); // value interpolated and mapped, so need ftemp
							fprintf(fp,"%.6g ",ftempC[m+1]); // value interpolated and mapped, so need ftemp
						}
						else{
							//	      fprintf(fp,"%21.15g ",ftempC[m+2]);
							fprintf(fp,"%.6g ",ftempC[m+2]);
						}
					}
